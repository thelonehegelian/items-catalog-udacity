...
"""
=========
This OAuth button for Google and Facebook code is mostly from instructors lessons and code provided in the project
=========
"""
...
def fbconnect():
    ...
    output = ''
    output += '<h1>Welcome to the Bibliomaniac, '
    output += login_session['username']

    output += '!</h1>'
    output += '<img src="'
    output += login_session['picture']
    output += ' " style = "width: 300px; height: 150px;border-radius: 150px;-webkit-border-radius: 150px;-moz-border-radius: 150px;"> '
...
def fbdisconnect():
    ...
    # The access token for logging out
...
# gconnect method - This does not work yet. Please ignore.
...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        response = make_response(json.dumps("Token user doesnt mathch the given id"), 401)
        response.headers['Content-type'] = 'application/json'
        return response
...
def addBook():
    if 'username' not in login_session:
        return redirect('/login')
    if request.method == 'POST':
        newBook = Book(category_id=request.form['category'], name = request.form['name'], description=request.form['description'])
        session.add(newBook)
        session.commit()
